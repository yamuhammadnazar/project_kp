                ...
                <div class="container-fluid">
                    ...
                    <ul class="nav nav-tabs mb-4" id="taskTabs" role="tablist">
                        <li class="nav-item" role="presentation">
                            <button class="nav-link active" id="anggota-tab" data-bs-toggle="tab"
                                data-bs-target="#anggota-tasks" type="button" role="tab" aria-controls="anggota-tasks"
                                aria-selected="true">
                        ...
                        <li class="nav-item" role="presentation">
                            <button class="nav-link" id="admin-tab" data-bs-toggle="tab" data-bs-target="#admin-tasks"
                                type="button" role="tab" aria-controls="admin-tasks" aria-selected="false">
                    ...
                    <div class="tab-content" id="taskTabsContent">
                        <!-- Tab Tugas Anggota -->
                        <div class="tab-pane fade show active" id="anggota-tasks" role="tabpanel"
                                    ...
                                    <div class="d-flex">
                                        ...
                                        <?php if ($jumlah_belum_dilihat > 0): ?>
                                            <a href="?filter=belum_dilihat" class="btn btn-outline-warning filter-btn me-2">
                                                ...
                                                <span
                                                    class="badge bg-danger notification-badge"><?= $jumlah_belum_dilihat ?></span>
                                            </a>
                                        <?php endif; ?>
                                                        ...
                                                        <tr>
                                                            ...
                                                            <td>
                                                                ...
                                                                <?php if (!empty($row['deskripsi'])): ?>
                                                                    <br><small
                                                                        class="text-muted"><?= substr(htmlspecialchars($row['deskripsi']), 0, 50) . (strlen($row['deskripsi']) > 50 ? '...' : '') ?></small>
                                                                <?php endif; ?>
                                                            </td>
                                                            <td><?= htmlspecialchars($row['platform']) ?></td>
                                                            <td>
                                                                <?php if (!empty($row['link_drive'])): ?>
                                                                    <a href="<?= htmlspecialchars($row['link_drive']) ?>"
                                                            ...
                                                            <td>
                                                                <?php if (isset($row['dilihat_kabid']) && $row['dilihat_kabid'] == 1): ?>
                                                                    <span class="badge bg-success"><i class="bi bi-eye-fill"></i>
                                                                        Sudah Dilihat</span>
                                                                    <?php if (!empty($row['waktu_dilihat_kabid'])): ?>
                                                                        <br><small
                                                                            class="text-muted"><?= date('d/m/Y H:i', strtotime($row['waktu_dilihat_kabid'])) ?></small>
                                                                    <?php endif; ?>
                                                                <?php else: ?>
                                                                    <span class="badge bg-warning"><i
                                                                            class="bi bi-eye-slash-fill"></i> Belum Dilihat</span>
                                                                <?php endif; ?>
                                                            </td>
                                                            <td>
                                                                <a href="../modules/konfirmasi_lihat_tugas_kabid.php?id=<?= $row['id'] ?>"
                                        ...
                                        <nav aria-label="Page navigation" class="mt-4">
                                            <ul class="pagination justify-content-center">
                                                <?php if ($anggota_page > 1): ?>
                                                    <li class="page-item">
                                                        <a class="page-link"
                                                            href="?anggota_page=<?= $i ?><?= isset($_GET['filter']) ? '&filter=' . $_GET['filter'] : '' ?><?= !empty($bulan) ? '&bulan=' . $bulan : '' ?><?= !empty($tahun) ? '&tahun=' . $tahun : '' ?>">
                                        ...
                                        <form class="d-flex me-2 align-items-center" method="GET">
                                            ...
                                            <select name="admin" class="form-select form-select-sm me-2">
                                                ...
                                                while ($admin_user = mysqli_fetch_assoc($admin_users_result)):
                                                    ?>
                                                    <option value="<?= $admin_user['username'] ?>"
                                                        <?= $admin == $admin_user['username'] ? 'selected' : '' ?>>
                                                        <?= $admin_user['username'] ?></option>
                                                <?php endwhile; ?>
                                            </select>
                                            <select name="status" class="form-select form-select-sm me-2">
                                                ...
                                                <option value="Kirim" <?= $status == 'Kirim' ? 'selected' : '' ?>>Kirim
                                                </option>
                                                <option value="Revisi" <?= $status == 'Revisi' ? 'selected' : '' ?>>Revisi
                                                </option>
                                                ...
                                                if (mysqli_num_rows($admin_tugas_result) > 0) {
                                                    ...
                                                    while ($row = mysqli_fetch_assoc($admin_tugas_result)) {
                                                        ...
                                                        <tr>
                                                            ...
                                                            <td>
                                                                <div class="btn-group">
                                                                    <a href="../views/catatan_kabid.php?id=<?php echo $row['id']; ?>"
                                                                        class="btn btn-sm btn-info">
                                                                        <i class="bi bi-eye"></i>
                                                                    </a>
                                                                    <a href="../modules/edit_tugas_kabid.php?id=<?php echo $row['id']; ?>"
                                                                        class="btn btn-sm btn-warning">
                                                                        <i class="bi bi-pencil"></i>
                                                                    </a>
                                                                    <a href="../modules/hapus_tugas_kabid.php?id=<?php echo $row['id']; ?>"
                                        ...
                                        <nav aria-label="Page navigation" class="mt-4">
                                            <ul class="pagination justify-content-center">
                                                <?php if ($admin_page > 1): ?>
                                                    <li class="page-item">
                                                        <a class="page-link"
                                                            href="?admin_page=<?= $i ?><?= !empty($admin) ? '&admin=' . $admin : '' ?><?= !empty($status) ? '&status=' . $status : '' ?>">
